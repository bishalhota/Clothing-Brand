...
const UserManagement = () => {
    ...
    useEffect(()=>{
        if(user && user.role !== "admin") {                                       //if user not admin then redirect to login page
            navigate("/login");
        }
    },[user,navigate])
    ...
    <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="p-6 rounded-lg mb-6">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button type='submit' className='bg-green-500 text-white py-2 px-2 rounded hover:bg-green-600'>Add User</button>
            </form>
        </div>
                ...
                <tbody>
                    {users.map((user) =>(
                        <tr key={user._id} className='border-b hover:bg-gray-50'>
                            ...
                            <td className='p-4'>
                                <button onClick={() => handleDelteUser(user._id)} className='bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600'>Delete</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
